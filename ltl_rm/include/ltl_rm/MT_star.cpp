...
/***djkistra with qtot system states with loops,until,next,few corrections,integer hash
 * improvements,consider node in productgraph for book-keeping***/
// correct the adj updations

clock_t t, t1, t2;

pair<int, int> create_automata_trans_table(char ltl_query[]) {
    cout<<"[MTSTAR::create_automata_trans_table]"<<endl;
    string table_text = "";
    ...
    while (i < table_text.size()) {
        ...
        while (i < table_text.size() && tmp != "fi;") {
            tmp = "";
            while (i < table_text.size() && table_text[i] != '\t' && table_text[i] != ' ' &&
                   table_text[i] != '\n') {
                if (table_text[i] != '(' && table_text[i] != ')')
                    tmp += table_text[i];
                i++;
            }
            if (tmp != " " && tmp != "" && tmp != "if" && tmp != "::" && tmp != "->" &&
                tmp != "goto" && tmp != "fi;")
                table.push_back(tmp);
            i++;
        }
    ...
    while (i < table.size()) {
        ...
        } else if (mp.find(table[i]) != mp.end())  // this is transition condition
        {
            ...
            for (x = 0; x < transition_condn_list.size(); x++) {
                ...
                outfile << automaton_state << " " << mp[table[i]] << " "
                        << transition_condn_list[x].size() << " ";
...
void initializegrid(const char* grid_info_file, std::string query_file) {
    cout << "[MTSTAR::initializegrid]" << endl;
    string s;
    int i, j, k, no_of_trans, literal, num_obstacles;
    char buff[1000];
    FILE* ft;
    // std::string name = path_prefix+"query.dat";
    ft = fopen(query_file.c_str(), "r");
    /*reading LTL query*/
    fgets(buff, 1000, (FILE*) ft);

    pair<int, int> automata_info;
    /**reading the automata***/
    automata_info = create_automata_trans_table(
        buff);  // generates trans.dat(transitions of Buchi Autometa)  * imp *
    ...
    for (i = 0; i < no_of_trans; i++) {
        ...
        if (neg_literals == trans_condn_len)  // All literal are -ve for a trans
        {
            if (automata_states[0] != automata_states[1] && strict_neg_literals == trans_condn_len)
                neg_trans_to_neighbour[automata_states[0]] =
                    1;  // stores node for which there is a -ve trans to some other node(all
                        // literals are -ve)
    ...
    for (int k = 0; k < No_Robots; k++) {
        ...
        vector<vector<vector<int>>> temp_prop_sys_states(
            10000);  // ## remember to update this as per TS space
        set<int> lit;
        grid_file >> num_pos_system_states;  // no of proposition system states where some
                                             // proposition is true.
        // cout << "propositions for robot " << k+1 << "has propositions true at " <<
        // num_pos_system_states << " states" << endl;
...
void generate_final_states() {
    cout << "[MTSTAR::generate_final_states]" << endl;
    ...
    for (unordered_set<int>::iterator f = dest.begin(); f != dest.end(); f++) {
        ...
        for (int j = 0; j < qtot; j++) {
            for (int z = 0; z < trans[j][final_B_state].size(); z++) {
                ...
                int r = pos_lit.size();
                if (r > 0)  // this is positive transition so there has to be a positive transtion
                            // so r is always greater than 1
                {
                    for (int nbh = 0; nbh < master_table[r - 1].size(); nbh++) {
                        ...
                        if (prop_state_count_list.find(v_prop_state_count_string) ==
                            prop_state_count_list.end())
                            generate_prop_state_count(
                                v_prop_state_count, v_prop_state_count_string);
...
void generate_abstract_reduced_graph() {
    cout << "[MTSTAR::generate_abstract_reduced_graph]" << endl;
    ...
    long long int ini_redc_no = ini_T_state_no * redc_node_multiplier +
                                ini_B_state;  // this is our start state in the Reduced graph
    ...
    while (!Q.empty()) {
        ...
        int negloop =
            0;  // I am not considering the SKIP transition  which may come in some Buchi Autometas
        // check whether exists a self transition onconjunction of neg literals of cur_B_no//
        if (negtrans[cur_B_no][cur_B_no].size() > 0)
            negloop = 1;

        for (int j = 0; j < qtot; j++) {
            for (int z = 0; z < trans[cur_B_no][j].size();
                 z++)  // considering outgoing Buchi Trans from curr Buchi state
            {
                ...
                for (int nbh = 0; nbh < master_table[r - 1].size(); nbh++) {
                    ...
                    for (int z = 0; z < per.size(); z++) {
                        ...
                        if (!neg_valid)
                            continue;
                        if (!negloop &&
                            !Are_neighbour(cur_T_no, state))  // Adding only neighbours(for 2 axis
    ...
    unordered_map<long long int, unordered_map<long long int, long long int>>::iterator i =
        Redc_E.begin();
    for (; i != Redc_E.end(); i++) {
        for (unordered_map<long long int, long long int>::iterator j = Redc_E[i->first].begin();
             j != Redc_E[i->first].end();
             j++) {
            count++;
        }
    }
...
long long int find_single_robot_path_cost(long long int src,
    long long int dest,
    int robot_no,
    vector<long long int>& cycle,
    int s_index,
    int e_index,
    vector<vector<vector<long long int>>>& actual_path) {
    cout << "[MTSTAR::find_single_robot_path_cost]" << endl;
    ...
    long long int src_no =
        (N_Joint_mapping[src / 1000][robot_no][1] * sz + N_Joint_mapping[src / 1000][robot_no][0]) *
            1000 +
        (s_index % 1000);  // ((y*sx+x)*1000) + B_state
    long long int dest_no = (N_Joint_mapping[dest / 1000][robot_no][1] * sz +
                                N_Joint_mapping[dest / 1000][robot_no][0]) *
                                1000 +
                            (e_index % 1000);
    ...
    if (N_Joint_mapping[src / 1000][robot_no][0] == N_Joint_mapping[dest / 1000][robot_no][0] &&
        N_Joint_mapping[src / 1000][robot_no][1] == N_Joint_mapping[dest / 1000][robot_no][1] &&
        src % redc_node_multiplier == dest % redc_node_multiplier) {
        // cout << " ****************";
    ...
    while (!qopen.empty()) {
        ...
        for (int j = 0;
             (!(path_valid) || ((cur_index) % cycle.size()) != e_index) &&
             j < Redc_E_Trans[cycle[cur_index]][cycle[(cur_index + 1) % cycle.size()]].size();
             j++)  // transition between transitionsa
        {
            vector<int> req =
                Redc_E_Trans[cycle[cur_index]][cycle[(cur_index + 1) % cycle.size()]][j][robot_no];
        ...
        {
            for (int j = 0; j < trans[cur_B_no][cur_B_no].size();
                 j++)  // transition between transitionsa
            {
...
long long int compute_cycle_cost(vector<long long int>& cycle,
    vector<vector<vector<long long int>>>& actual_path) {
    cout << "[MTSTAR::compute_cycle_cost]" << endl;
    ...
    for (int i = 0; i < No_Robots; i++) {
        ...
        for (; j != first_loc; j = (j + 1) % cycle.size()) {
            ...
            if (memo[i].find(tmp) != memo[i].end()) {
                ...
                long long int cost = find_single_robot_path_cost(
                    cycle[src_loc], cycle[dest_loc], i, cycle, src_loc, dest_loc, actual_path);
                S_cost += cost;
                memo[i][tmp] = cost;
            }
        ...
        if (memo[i].find(tmp) != memo[i].end())
            ...
            long long int cost = find_single_robot_path_cost(
                cycle[src_loc], cycle[first_loc], i, cycle, src_loc, first_loc, actual_path);
            S_cost += cost;
            memo[i][tmp] = cost;
        }
...
void DFS(long long int src_no,
    long long int cur_no,
    long long int dest_no,
    unordered_set<long long int> visited,
    vector<long long int> cycle,
    int degen) {
    cout << "[MTSTAR::DFS]" << endl;
    for (unordered_map<long long int, long long int>::iterator i = Redc_E[cur_no].begin();
         i != Redc_E[cur_no].end();
         i++) {
        if (i->first == dest_no) {
            Total_cycles++;
            ...
            if (c < min_suffix_cost) {
                ...
                for (int i = 0; i < pseudo_suffix_cycle.size(); i++) {
                    for (int j = 0; j < pseudo_suffix_cycle[i].size(); j++) {
                        for (int k = 0; k < pseudo_suffix_cycle[i][j].size(); k++) {
                            int temp = pseudo_suffix_cycle[i][j][k] / 1000;
                            int nx = temp % 10, ny = temp / sz;
                        }
                    }
                }
...
vector<vector<pair<double, double>>> mt_star(std::string map_file = "",
    std::string query_file = "",int no=-1) {
    cout << "[MTSTAR::mt_star]" << endl;
    t = clock();
    // char newmap [][20] =
    // {"mapa_1.dat","mapa_2.dat","mapa_3.dat","mapa_4.dat","null","mapg_1.dat","mapg_2.dat","mapg_3.dat"};
    // initializegrid(argv[1]);
    // int k=(phi*5)+no_robot;
    //  std::string name = path_prefix+std::string(newmap[k-1]);
    // std::string name=path_prefix+"map_11.dat";
    // std::cout<<"name " << name << "phi" <<  phi<<std::endl;
    initializegrid(map_file.c_str(), query_file);
    // char newmap[20] = "mapg_3.dat";
    // initializegrid(argv[1]);
    // initializegrid(newmap);
    generate_final_states();
    generate_abstract_reduced_graph();
    int count = 0;
    for (unordered_set<int>::iterator x = dest.begin(); x != dest.end(); x++)
        cout << *x << endl;
    for (unordered_set<long long int>::iterator i = Final_states.begin(); i != Final_states.end();
         i++) {
    ...
    cout << endl << endl;
    vector<vector<pair<double, double>>> trajectory(No_Robots);
    for (int r = 0; r < No_Robots; r++) {
        ...
        for (int i = 0; i < min_suffix_cycles[0].size(); i++) {
            ...
            if (x == -1){
                cout << "(*"<< " "<< "*)";
               // trajectory[r].push_back({-1, -1});
            }
                else {
                    cout << "(" << x << " " << y << ")";
                    trajectory[r].push_back({x, y});
            }
            cout << " --> ";
        }
    ...
    cout << endl;
    cout << "............................................................................." << endl;
    int robot_max=INT_MIN;
    for (int i = 0; i < trajectory.size(); i++) {
        cout << "Robot " << i << " Trajectory\n";
        int j = trajectory[i].size();
        if(j<1) continue;
        if (j > 1 and trajectory[i][0] == trajectory[i][1]) {
            trajectory[i].erase(trajectory[i].begin()+1,trajectory[i].end());
        }
        else if (!(trajectory[i][0].first == trajectory[i][j - 1].first and
        ...
        for (int j = 0; j < trajectory[i].size(); j++) {
            ...
            robot_max=max(j,robot_max);
        }
        cout << "\n";
    }
    cout << "max trajectory size "<<robot_max+1<<"\n";
    sz = 10;
    qtot = 3;
    min_suffix_cycles.clear();
    pseudo_suffix_cycle.clear();
    dir = 5;
    total_system_states = 0;
    comp_red = 0, tot_comp = 0;
    grid.clear();
    trans.clear();
    negtrans.clear();
    neg_trans_to_neighbour.clear();
    dest.clear();
    dim = 2;
    No_Robots = 1;
    max_no_robots = 11;
    rob_pos_system_state.clear();
    rob_prop.clear();
    rob_prop_sys_states.clear();
    rob_literal.clear();
    INI_LOC.clear();
    redc_node_multiplier = 1000;
    Neighbours.clear();
    Permutations.clear();
    max_propositions_in_formulae = 8;
    n_c_r.clear();
    prop_state_count_list.clear();
    cur_joint_size = 0;
    N_Joint_N.clear();
    N_Joint_mapping.clear();
    N_Joint_E.clear();
    N_Updated.clear();
    N_Reverse_Joint_N.clear();
    Redc_N.clear();
    Redc_E.clear();
    Redc_E_Trans.clear();
    Final_states.clear();
    ...
    mpl = 8;
    tasks.clear();
    tasks.resize(mpl, vector<vector<vector<vector<int>>>>(mpl));
    master_table.clear();
    master_table.resize(mpl);
    max_no_of_prop_def = 100;
    prop_bit_map.clear();
    min_suffix_cost = 5000000;
    memo.clear();
    return trajectory;
}
// int main(int args,char **argv){
// 	vector<vector<pair<int,int>>> trajectory=run(args,argv);
// 	cout<<"Printing via Calling Function"<<endl;
// 	for(int i=0;i<trajectory.size();i++){
// 	cout<<"Robot "<<i<<" Trajectory\n";
// 	for(int j=0;j<trajectory[i].size();j++){
// 		cout<<"("<<trajectory[i][j].first<<","<<trajectory[i][j].second<<") ";

#include <bits/stdc++.h>
#ifndef MTPLANNER_VARIABLES_H
#    define MTPLANNER_VARIABLES_H
using namespace std;
...
vector<vector<vector<vector<int>>>>
    rob_prop_sys_states;  // ## remember to update this as per TS space
...
vector<vector<vector<int>>>
    n_c_r;  // n: No of robots    r: propositions that must me satisfied by the robots
unordered_map<string, vector<vector<int>>> prop_state_count_list;

long long int cur_joint_size = 0;
unordered_map<long long int, int>
    N_Joint_N;  // node secret no with valid index 0: valid , -1: Invalid
...
unordered_map<long long int, unordered_map<long long int, long long int>>
    N_Joint_E;  // E[From_node][To_node] = weight
unordered_map<long long int, unordered_map<long long int, bool>>
    N_Updated;  // E[From_node][To_node] = True/False

unordered_map<string, long long int> N_Reverse_Joint_N;
unordered_map<long long int, int> Redc_N;  // node values
unordered_map<long long int, unordered_map<long long int, long long int>>
    Redc_E;  // E[From_node][To_node] = weight
unordered_map<long long int, unordered_map<long long int, vector<vector<vector<int>>>>>
    Redc_E_Trans;  // E[From_node][To_node] = weight
...
vector<unordered_map<vector<bool>, vector<int>>>
    prop_bit_map;  //  list per robot of  [bitmap] -> coord(y * sz + x)
...
bool Are_neighbour(long long int node, vector<vector<int>>& state) {
    for (int i = 0; i < No_Robots; i++) {
        ...
        if (N_Joint_mapping[node][i][0] == -1)
            continue;
        if (abs(state[i][0] - N_Joint_mapping[node][i][0]) +
                abs(state[i][1] - N_Joint_mapping[node][i][1]) >
            1)
            return false;
    }
...
void prop_power_set_mapping(vector<int>& prop_set,
    vector<bool> P,
    int index,
...
void generate_prop_power_set_mapping() {
    for (int i = 0; i < No_Robots; i++) {
        unordered_map<vector<bool>, vector<int>> sol;
        for (unordered_map<int, vector<int>>::iterator itr = rob_prop[i].begin();
             itr != rob_prop[i].end();
             itr++) {
            int coord = itr->first;
    ...
    // cout << "Printing  prop_bit_map
    // ---------------------------------------------------------------" <<endl; for( int i = 0; i <
    // No_Robots; i++)
    // {
    //     for( unordered_map< vector<bool> , vector<int> >::iterator itr = prop_bit_map[i].begin();
    //     itr != prop_bit_map[i].end(); itr++ )
    //     {
                ...
                for (int n = 0; n < n_c_r[t - 1].size(); n++) {
                    ...
                    for (int p = 0; p < Permutations[t - 1].size(); p++) {
                        ...
                        for (int l = 0; l < locs.size(); l++) {
                            // cout << "I am here 2 " <<  locs[l] << " "  << Permutations[t-1][p][l]
                            // << endl;
